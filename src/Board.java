...
//wczytaj zapisana macierz punktow
    public void loadMap()
    {
        ...
        for (Map.Entry<Pkt, P> o : Core.pkts.entrySet())
        {
...
//exportuj tablice punktow
    ...
    public void exportToFile()
    {
        ...
        try {
            ...
            out.newLine();
            for (Map.Entry<Pkt, P> entry : Core.pkts.entrySet())
            {
        ...
        try
        {
            while ((line = reader.readLine()) != null)
            {
                ...
                if (parts.length >= 6)
                {
                    ...
                    if (actInt == 1)
                    {
                        Core.pkts.put(new Pkt(xx, yy), new P(actInt,prevInt,integerRGBInt));
                    }
    ...
    public void addInclusionsAfter()
    {
        ...
        int[][] tabOfBoundaryGrains = new int[Core.pkts.size()*50][2];
        ...
        while(numberOfInclusionsBegining<Core.Config.numberOfInclusions)
        {
            ...
            for (Map.Entry<Pkt, P> entry : Core.pkts.entrySet())
            {
                ...
                if(Core.Config.rangeOfInclusions != 0)
                {
                    if(!Core.Config.circleInclusions)
                        for (int i = 0; i < Core.Config.rangeOfInclusions; i++)
                        {
                            for (int j=0; j<Core.Config.rangeOfInclusions; j++)
                            {
                    ...
                    else
                    {
                        ...
                        int r = Core.Config.rangeOfInclusions;
...
//        this.saveMap();
//
//        savedEntries = Core.pkts.entrySet();
...
//                entry.getValue().setWhite();
    ...
    public void addInclusionsAfterRandom()
    {
        ...
        Object[] values = Core.pkts.entrySet().toArray();
    ...
    public void addSubstructureVer1()
    {
        ...
        Set<Map.Entry<Pkt, P>> entries = Core.pkts.entrySet();
